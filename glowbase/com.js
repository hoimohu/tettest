    ...
    function minobagNext() {
      if (game) {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          nextc[i].items = [
            new imgObj(canvas, 0, 0, canvas.width, canvas.height),
          ];
        }
        if (holdMino != null) {
...
function battle() {
  if (gamemode !== "battle") {
    ...
    damagec.items = [
      new imgObj(dMain.canvas, 0, 0, dMain.canvas.width, dMain.canvas.height),
    ];
    ...
    pfieldc = new charObj(
      "calc(100%-(50@+10*100%/100@*100%/100@))",
      50,
      false,
      80
    );
