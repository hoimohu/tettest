...
class mino {
    ...
    constructor(type, fi, landFunction) {
        ...
        this.field = fi;
        this.landPermission = false;
        this.inter = [setInterval(this.minodown.bind(this), 1000)];
    ...
    control(angle) {
        if (this.landing === false) {
            if (angle === ' ') {
            ...
            } else if (angle === 'ArrowUp') {
                this.rotate(1);
            } else if (angle === 'z') {
                this.rotate(-1);
            } else if (angle === 'ArrowRight') {
                let permission = true;
                for (let i = 0; i < this.data.length; i++) {
                    const e = this.data[i];
                    if (this.type === 'i' || this.type === 'o') {
                ...
                if (permission === false) {
                    ...
                    while (this.inter.length !== 0) {
                        clearInterval(this.inter.shift());
                    }
                    this.inter.push(setInterval(this.minodown.bind(this), 1000));
                    this.tspin = false;
                    this.miniTspin = false;
                }
            } else if (angle === 'ArrowLeft') {
                let permission = true;
                for (let i = 0; i < this.data.length; i++) {
                    const e = this.data[i];
                    if (this.type === 'i' || this.type === 'o') {
    ...
    minodown() {
        if (this.landing === false) {
            ...
            if (permission === false && this.landPermission) {
                while (this.inter.length !== 0) {
                    clearInterval(this.inter.shift());
                }
                this.landing = true;
